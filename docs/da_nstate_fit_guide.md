# DA N-State Fit Guide

This note collects the current best-practice workflow for the DA `n-state` ratio-fit pipeline in this repository.

The goal is practical use:

- run the fit in a stable order
- choose the two-point reference windows with clear intent
- understand how the DA ratio is built from the correlators and the two-point fit
...
The DA `n-state` workflow fits a ratio:

```text
ratio(t) = DA_numerator(t) / two_point_correlator(t)
```

The numerator is built from the extracted DA matrix element and the selected operator channel:
...
The DA workflow uses three groups of inputs.
...
- `qda_h5` may use `{gm}` and `{pz}` placeholders through the notebook helper.
...
The workflow uses this two-point fit table to supply fixed amplitudes and energies for the DA ratio model by default.
If you enable `two_point_fit_sample_coupled`, it instead reads the matching bootstrap sample row from the two-point sample table for each DA bootstrap sample.
...
The `fit_window` input is the main tuning knob for the DA ratio fit.
...
- `tmax` is secondary, but it should still stay in a region where the DA/2pt ratio does not show obvious loss of signal or strange behavior
- the best `tmax` for the DA ratio fit can differ from the two-point fit `tmax`
...
- conceptually, the scan is organized as: pick a two-point reference `tmin`, then scan the DA `fit_window` around it
...
The normal DA `n-state` workflow writes grouped outputs by `bT`.
...
- if the selected DA result behaves poorly, try a nearby two-point window first
...
- for the initial `fit_window`, usually start with a `tmin` equal to the chosen two-point `tmin` or a little larger, and choose `tmax` by requiring the DA/2pt ratio to stay smooth and non-odd
...
The DA `n-state` workflow is a ratio fit with fixed two-point inputs.
The two most important human decisions are:

1. choosing a stable two-point reference window
2. choosing a DA fit window that suppresses excited-state contamination
