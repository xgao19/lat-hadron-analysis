...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 说清楚这个样例的输入、输出、中间文件、关键图和主要物理检查（尤其是 effective mass plateau 诊断，这对应后续 Correlator Analyst Agent 的职责）；
...
\paragraph{Week 1 常见错误}
\begin{itemize}[leftmargin=2em]
  \item \textbf{只跑 notebook，不记录命令}：必须记录每一条命令和对应的输出。
  \item \textbf{一开始就让 Codex 写完整系统}：每天只做一个小任务，不要一次要求 Codex 完成整个 Month 1。
  \item \textbf{不理解 effective mass 就继续}：如果看不出来 plateau，先回看 Week 1 的物理背景部分。
\end{itemize}

\paragraph{Week 1 导师检查问题}
\begin{enumerate}[leftmargin=2em]
  \item 学生是否真的跑通了一遍 golden example？
  \item 是否记录了完整命令和 git commit hash？
  \item 是否理解了主要输入输出？
...
\subsection{Week 2 命令示例}

\begin{lstlisting}[language=bash]
python -m lqcd_agent.cli.run_golden --config configs/golden_example.yaml
pytest tests/test_schema.py tests/test_cli_smoke.py -q
\end{lstlisting}

\subsection{Week 2 常见错误}
\begin{itemize}[leftmargin=2em]
  \item \textbf{config 字段太随意}：所有影响结果的参数必须进 config，用 Pydantic Field 加约束。
  \item \textbf{没有 schema 直接读 dict}：后续 agent 无法可靠读取 config/result。
  \item \textbf{CLI 没有 overwrite protection}：默认不应覆盖已有输出。
\end{itemize}

\subsection{Week 2 导师检查问题}
\begin{itemize}[leftmargin=2em]
...
\subsection{Week 3 任务 1：data audit 模块（Data Integrity Check / Correlator Analyst）}
...
\begin{lstlisting}
Please implement a data audit module (Data Integrity Check) for the golden example.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item 测试覆盖正常和失败情况；
  \item 学生理解为什么 data audit 是 Analysis Engineer 之前的第一步。
\end{itemize}
...
3. data audit module fails when input_root is missing;
...
\subsection{Week 3 任务 2.5：生成 correlator\_diagnostics.json}

基于 effective mass 和 bootstrap 结果，生成 Correlator Analyst 的第一版输出：

\begin{lstlisting}
outputs/<case_id>/correlator_diagnostics.json
outputs/<case_id>/effective_mass_summary.csv
outputs/<case_id>/effective_mass.pdf
\end{lstlisting}

\texttt{correlator\_diagnostics.json} 至少包含：每个 momentum 的 plateau 检测结果、bootstrap 误差评估、标记为 suspicious 的 correlator（如果存在）。
...
\subsection{Week 3 常见错误}
\begin{itemize}[leftmargin=2em]
  \item \textbf{用真实数据路径写测试}：测试会不可迁移；用 \texttt{tmp\_path} 创建临时目录。
  \item \textbf{data audit 不检查 output dir}：可能意外覆盖已有结果。
  \item \textbf{correlator diagnostics 只画图不输出结构化 JSON}：图是给人看的，JSON 是给后续 agent 读的。
\end{itemize}

\subsection{Week 3 导师检查问题}
\begin{itemize}[leftmargin=2em]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item 学生能说明为什么 raw data 不能被 Analysis Engineer 修改；
  \item 输出目录结构稳定。
\end{itemize}

\section{Week 4：Month 1 集成、最小 Analysis Engineer 雏形与展示}

\subsection{Week 4 目标}

第四周的目标是完成第一个月的集成展示：学生应该能用一个命令运行 golden example，并生成完整的结构化输出和复现报告。同时，开始引入 Analysis Engineer Agent 的雏形，但不要求完成完整多 agent 系统。
...
\begin{itemize}[leftmargin=2em]
  \item Analysis Engineer Agent 的边界；
...
\subsection{Week 4 任务 1：定义 Analysis Engineer task schema}

创建：

\begin{lstlisting}
src/lqcd_agent/schema/analysis_engineer_task.py
\end{lstlisting}
...
class AnalysisEngineerTaskResult(BaseModel):
    task_id: str
    status: str
...
\begin{lstlisting}
Please add a minimal Analysis Engineer task schema.

File:
`src/lqcd_agent/schema/analysis_engineer_task.py`

Requirements:
- define AnalysisEngineerTask with task_id, task_type, config_path, output_dir, allow_overwrite, notes;
- define AnalysisEngineerTaskResult with status, output paths, warnings, errors;
...
\subsection{Week 4 任务 2：最小 Analysis Engineer wrapper}
...
Analysis Engineer wrapper 的第一版职责：

\begin{enumerate}[leftmargin=2em]
  \item 读取 AnalysisEngineerTask；
  \item 检查 task type；
  \item 调用 \texttt{run\_golden} CLI 或内部函数；
  \item 检查输出文件是否存在；
  \item 写 AnalysisEngineerTaskResult；
...
\begin{lstlisting}
Please implement a minimal Analysis Engineer wrapper for Month 1.
...
- load AnalysisEngineerTask schema;
...
- write `analysis_engineer_result.json`;
...
\begin{itemize}[leftmargin=2em]
  \item Analysis Engineer task 可以运行 golden example；
  \item \texttt{worker\_result.json} 生成；
  \item 输出缺失时 Analysis Engineer 能报告失败；
  \item Analysis Engineer 不做 fit selection 或 final physics decision；
  \item 测试通过。
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item data audit；
  \item pytest 测试列表；
  \item Analysis Engineer wrapper 雏形；
  \item 当前限制；
  \item Month 2 计划。
\end{enumerate}

\subsection{Week 4 命令示例}

\begin{lstlisting}[language=bash]
# 一键运行 Analysis Engineer
python -m lqcd_agent.analysis_engineer.run \\
  --task configs/tasks/month1_golden_task.yaml
# 检查输出
ls outputs/l64c64a076_m140/
cat outputs/l64c64a076_m140/correlator_diagnostics.json
# 运行全部测试
pytest -q
\end{lstlisting}

\subsection{Week 4 常见错误}
\begin{itemize}[leftmargin=2em]
  \item \textbf{把 Analysis Engineer 当成物理判断者}：它只执行和检查，不能决定 central fit。
  \item \textbf{输出没有 provenance}：图和数值如果没有 config、git commit、run id 和 log，无法用于严肃科研。
  \item \textbf{Month 1 final report 只列做了什么，不写学到什么}：应该包含 failures 和 lessons。
\end{itemize}

\subsection{Week 4 导师检查问题}
\begin{itemize}[leftmargin=2em]
  \item 是否能从 clean repo 一键运行 Month 1 全流程？
  \item pytest 是否全部通过？学生是否能解释每个测试保护什么？
  \item 学生能否解释 Analysis Engineer 的作用和边界？
  \item Month 1 report 是否包含复现命令、输出文件列表、failures 和 lessons？
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item 一页展示 pytest 结果；
  \item 一页说明 Analysis Engineer wrapper；
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item data audit 模块为什么不能修改 raw data？
  \item Analysis Engineer 为什么不能决定 central physics result？
...
\subsection{错误 5：把 Analysis Engineer 当成物理判断者}

Analysis Engineer 可以运行和整理结果，但不能决定 central fit、丢弃数据或给出最终物理结论。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 增加 Fit Strategy Postdoc、Matrix Element Analyst 和 Critical Referee；
  \item 建立 fit candidate summary table；
  \item 实现 rule-based Analysis Coordinator；
...
\begin{lstlisting}
config -> schema -> CLI -> audit -> output -> tests -> Analysis Engineer wrapper
\end{lstlisting}

只有这条链路清楚，Month 2 的 Snakemake 和 Analysis Coordinator 才能自然接上。
...
\begin{itemize}[leftmargin=2em]
  ...
  \item 能写最小 pytest；
  \item 能用 Git 做小步提交；
  \item 能解释 Analysis Engineer 的作用和边界；
  \item 能用一个命令运行基础 pipeline；
  \item 能提交一份清楚的 Month 1 final report。
\end{itemize}

如果学生完成以上内容，就已经具备继续进入 Month 2 高级 workflow 和 Analysis Coordinator agent 阶段的基础。
