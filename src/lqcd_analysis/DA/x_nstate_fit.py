...
def parse_da_x_nstate_fit_input(
    path: str | Path,
    *,
    ...
    if not nstates or any(state not in {1, 2} for state in nstates):
        raise ValueError("DA x-space nstates must contain only 1 and/or 2")
    ...
    return DAXNStateFitInput(
        ...
        results_dir=(
            ...
            else Path(entries.get("results_dir", [file_path.parent / "results_da_x_nstate_fit"])[0])
        ),
    )
...
def resolve_ratio_fourier_t_sample_path(
    ...
    *,
    ...
    if not path.exists():
        raise FileNotFoundError(f"DA ratio-Fourier-t sample table does not exist: {path}")
    return path
...
def _fit_curves_for_x(
    ...
    *,
    ...
    return np.asarray(
        [
            np.real(evaluate_da_ratio(times, amplitudes, energies, params, nt, gm=gm, pz=pz, ns=ns))
...
def plot_da_x_nstate_fit_diagnostics(
    ...
    *,
    ...
    # The DA time model is defined for integer times. Use integer support
...
def write_da_x_nstate_fit_plot_notebook(
        ...
        "cells": [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["# DA x-space N-state Fit Diagnostics\n"],
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "import numpy as np\n",
                    "from lqcd_analysis.DA.io import expand_template, resolve_two_point_fit_reference\n",
                    "from lqcd_analysis.DA.x_nstate_fit import (\n",
                    "    load_ratio_fourier_t_samples,\n",
                    "    plot_da_x_nstate_fit_diagnostics,\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "                outputs.append(plot_da_x_nstate_fit_diagnostics(\n",
...
def run_da_x_nstate_fit_workflow(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_da_x_nstate_fit_input(input_file, results_dir=results_dir)
            ...
            for eta in spec.etalist:
                ...
                for bT in spec.bTlist:
                    ...
                    for nstates in spec.nstates:
                        ...
                        outputs.append(
                            write_da_x_nstate_fit_plot_notebook(
