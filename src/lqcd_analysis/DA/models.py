...
def normalize_da_operator(gm: str) -> str:
    normalized = gm.strip().upper()
    if normalized not in {"T5", "Z5"}:
        raise ValueError(f"unsupported DA operator label: {gm}")
    return normalized
...
def evaluate_da_numerator(
    ...
    *,
    ...
    if operator == "T5":
        return evaluate_da_numerator_t5(times, amplitudes, energies, matrix_elements, nt)
    return evaluate_da_numerator_z5(times, amplitudes, energies, matrix_elements, nt, pz=pz, ns=ns)


def evaluate_da_ratio(
    times: np.ndarray,
    amplitudes: np.ndarray,
    energies: np.ndarray,
    matrix_elements: np.ndarray,
    ...
    *,
    ...
    numerator = evaluate_da_numerator(times, amplitudes, energies, matrix_elements, nt, gm=gm, pz=pz, ns=ns)
