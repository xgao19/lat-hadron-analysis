...
def parse_da_fourier_input(
    ...
    *,
    ...
    if not input_root.exists():
        raise FileNotFoundError(f"DA Fourier input_root does not exist: {input_root}")

    output_root = Path(results_dir) if results_dir is not None else Path(entries.get("results_dir", [file_path.parent / "results_da_fourier"])[0])

    return DAFourierInput(
...
def _resolve_fit_sample_paths(
    ...
    if not tables_dir.exists():
        raise FileNotFoundError(f"DA Fourier tables directory does not exist: {tables_dir}")
    if not samples_dir.exists():
        raise FileNotFoundError(f"DA Fourier samples directory does not exist: {samples_dir}")
...
def resolve_da_fourier_output_paths(
    ...
    *,
    ...
    if not table_path.exists():
        raise FileNotFoundError(f"DA Fourier table does not exist: {table_path}")
    if not sample_path.exists():
        raise FileNotFoundError(f"DA Fourier sample table does not exist: {sample_path}")
...
def load_da_m0_fit_table(path: str | Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def load_da_m0_sample_table(path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    header, rows = _parse_grouped_table(path)
    index = {name: idx for idx, name in enumerate(header)}
...
def compute_da_cosine_transform(
...
def summarize_da_fourier_samples(samples: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def load_da_fourier_sample_table(path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    header, rows = _parse_grouped_table(path)
    index = {name: idx for idx, name in enumerate(header)}
    required = {"sample_id", "x", "q_sample"}
    missing = required - index.keys()
    if missing:
        raise ValueError(f"DA Fourier sample table is missing columns: {sorted(missing)}")
...
def run_da_fourier_from_fit_outputs(
    *,
    output_root: str | Path,
    stem: str,
    fit_table: str | Path,
    ...
    bz_fit, mean_fit, err_fit = load_da_m0_fit_table(fit_table)
    bz_samples, m0_samples = load_da_m0_sample_table(sample_table)
    if not np.array_equal(bz_fit, bz_samples):
        raise ValueError("fit/sample bz grids do not match for DA Fourier transform")

    x_grid = np.asarray(DEFAULT_X_VALUES if x_values is None else x_values, dtype=float)
    if m0_samples.shape[0] > 0:
        _, q_samples = compute_da_cosine_transform(
            bz_fit,
            m0_samples,
            pz=pz,
            ns=ns,
        ...
        q_mean, q_err, q_p16, q_p84 = summarize_da_fourier_samples(q_samples)
    else:
        _, q_mean_only = compute_da_cosine_transform(
            bz_fit,
            mean_fit[None, :],
            pz=pz,
            ns=ns,
...
def run_da_fourier_workflow(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_da_fourier_input(input_file, results_dir=results_dir)
    outputs: list[Path] = []
