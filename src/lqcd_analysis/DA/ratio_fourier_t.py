...
from .fourier import DEFAULT_INTERPOLATION_KIND, DEFAULT_X_VALUES, DEFAULT_ZSTEP_FM, compute_da_cosine_transform
from .io import _load_da_correlator_from_handle, expand_template, resolve_qda_h5_path
from .models import normalize_da_operator


@dataclass(frozen=True)
class DARatioFourierTInput:
    title_pattern: str
    ns: int
    nt: int
...
def parse_da_ratio_fourier_t_input(
    path: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> DARatioFourierTInput:
    file_path = Path(path)
    ...
    return DARatioFourierTInput(
        ...
        results_dir=(
            ...
            else Path(entries.get("results_dir", [file_path.parent / "results_da_ratio_fourier_t"])[0])
        ),
    )
...
def run_da_ratio_fourier_t_workflow(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_da_ratio_fourier_t_input(input_file, results_dir=results_dir)
    outputs: list[Path] = []
                ...
                for eta in spec.etalist:
                    for bT in spec.bTlist:
                        ...
                        for bz in spec.bzlist:
                            numerator = _load_da_correlator_from_handle(
                                qda_handle,
                        ...
                        for t_index in range(component_samples.shape[2]):
                            _, q_samples_t = compute_da_cosine_transform(
