...
from .models import evaluate_da_ratio, normalize_da_operator
from .plotting import RatioFitPlotSeries, plot_da_m0_from_fit_tables, plot_da_ratio_fit, write_da_plot_notebook
...
def parse_da_fit_input(path: str | Path, results_dir: str | Path | None = None) -> DANStateInput:
    file_path = Path(path)
    ...
    if not nstates or any(state not in {1, 2} for state in nstates):
        raise ValueError("DA nstates must contain only 1 and/or 2")
    for gm in entries["gmlist"]:
        normalize_da_operator(gm)
    ...
    return DANStateInput(
        ...
        results_dir=(
            ...
            else ((input_path.parent / "results_da_fit") if results_dir is None else Path(results_dir))
        ),
    )
...
def fit_da_component(
    ...
    def residuals(
        ...
        model_values = evaluate_da_ratio(
            prepared.times,
            fit_amplitudes,
            fit_energies,
            params,
...
def _write_decay_constant_check_summary(
    ...
    stem: str,
    records: tuple[DAOutputRecord, ...],
    base_fit_records: tuple[DAOutputRecord, ...],
    *,
    lattice_spacing_fm: float,
    pz: int,
    gm: str,
    ...
    with summary_path.open("w", encoding="utf-8") as handle:
        ...
        sortable_records: list[tuple[float, DAOutputRecord, float, float]] = []
        for record in records:
            _, _, m0_gev_mean, m0_gev_err = _summarize_ground_state_matrix_element(
                record.sample_params,
                lattice_spacing_fm=lattice_spacing_fm,
...
def run_da_nstate_fit(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_da_fit_input(input_file, results_dir=results_dir)
    if spec.two_point_fit_sample_coupled is None:
        print(
            "[da-fit] WARNING: two_point_fit_sample_coupled is not set; using summary-table two-point references"
        )
    outputs: list[Path] = []
    ...
    for pz in spec.pzlist:
        ...
        try:
            ...
            raise ModuleNotFoundError("h5py is required to load DA HDF5 data") from exc

        for gm in spec.gmlist:
            qda_path = resolve_qda_h5_path(spec.qda_h5, pz=pz, gm=gm)
            with h5py.File(qda_path, "r") as qda_handle:
                for eta in spec.etalist:
                    if spec.decay_constant_check:
                        decay_constant_records: list[DAOutputRecord] = []
                        ...
                        bT = 0
                        bz = 0
                        numerator_selected = _load_da_correlator_from_handle(
                            qda_handle,
                        ...
                        for scan_two_point_tmin in two_point_tmin_values:
                            for nstates in spec.nstates:
                                ...
                                for scan_tmin, scan_tmax in scan_windows:
                                    fit_result, sample_params = fit_da_component(
                                        ratio_samples,
                                        fit_amplitudes,
                                        fit_energies,
                                        spec.nt,
                    ...
                    for bT in selected_bT_values:
                        grouped_records: dict[tuple[str, int], list[DAOutputRecord]] = {}
                        grouped_ratio_records: list[DARatioRecord] = []
                    ...
                    notebook_path = notebook_root / f"{title}_{sanitize_token(gm)}_{sanitize_token(eta)}_da_plots.ipynb"
                    outputs.append(
