...
from .plotting import plot_da_m0_from_fit_tables
from ..common.parsing import parse_int_list_or_range


@dataclass(frozen=True)
class DANormalizeInput:
    title_pattern: str
    input_root: Path
    ns: int
...
def parse_da_normalize_input(
    ...
    *,
    ...
    output_root = (
        ...
        else Path(entries.get("results_dir", [file_path.parent / "results_da_normalized"])[0])
    )
    return DANormalizeInput(
...
def _resolve_fit_sample_paths(
    ...
    if not fit_path.exists():
        raise FileNotFoundError(f"DA normalization fit table does not exist: {fit_path}")
    if not sample_path.exists():
        raise FileNotFoundError(f"DA normalization sample table does not exist: {sample_path}")
...
def _require_reference_bz0(
    ...
    *,
    ...
    if 0 not in row_map:
        raise ValueError(f"DA normalization missing bz=0 reference row for {label}")
    available = any(0 in by_bz for by_bz in sample_map.values())
    if not available:
        raise ValueError(f"DA normalization missing bz=0 reference samples for {label}")
...
def run_da_normalization(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_da_normalize_input(input_file, results_dir=results_dir)
    outputs: list[Path] = []
                ...
                for bT in spec.bTlist:
                    ...
                    for bz in spec.bzlist:
                        if bz not in target_rows:
                            raise ValueError(
                                f"DA normalization missing target bz={bz} in {target_fit_path}"
                            )
