...
def compute_pairwise_type2_estimators(
    ...
    *,
    ...
    for comparison, p2_gev in zip(comparison_samples, p2_gevs):
        ...
        values = 1.0 / np.log(p1_gev / p2_gev) * np.log(np.abs(reference_samples / comparison)) + correction
        if not np.all(np.isfinite(values)):
            raise ValueError(
                "non-finite type-2 CS-kernel estimator encountered; "
                f"check the bootstrap samples near x={x_value:.6f}, P1={p1_gev:.6f}, P2={p2_gev:.6f}"
            )
        q16, _, q84 = _legacy_quantile_triplet(values)
        sigma = 0.5 * (q84 - q16)
        pair_samples.append(np.asarray(values, dtype=float))
        pair_sigmas.append(float(max(sigma, np.finfo(float).eps)))
    return np.asarray(pair_samples, dtype=float), np.asarray(pair_sigmas, dtype=float)


def fit_constant_across_p2(sample_values: np.ndarray, sigma: np.ndarray) -> tuple[float, float]:
    values = np.asarray(sample_values, dtype=float)
    errors = np.asarray(sigma, dtype=float)
    if values.shape != errors.shape:
        raise ValueError("sample_values and sigma must have the same shape")

    def residuals(params: np.ndarray) -> np.ndarray:
        return (values - params[0]) / errors

    result = least_squares(residuals, np.array([0.0]), method="lm")
    dof = values.size - 1
    chi2_dof = float(np.sum(result.fun**2) / dof) if dof > 0 else float("nan")
    return float(result.x[0]), chi2_dof
...
def extract_cs_kernel_for_reference(
    ...
    *,
    ...
    for local_index, x_value in zip(np.where(x_mask)[0], x_values):
        ...
        pair_samples, pair_sigmas = compute_pairwise_type2_estimators(
            reference_samples,
            comparison_samples,
            scheme=scheme,
            kernel_label=kernel_label,
        ...
        gamma_samples = np.empty(reference.samples.shape[0], dtype=float)
        chi2_samples = np.empty(reference.samples.shape[0], dtype=float)
        for sample_id in range(reference.samples.shape[0]):
            gamma_samples[sample_id], chi2_samples[sample_id] = fit_constant_across_p2(pair_samples[:, sample_id], pair_sigmas)
