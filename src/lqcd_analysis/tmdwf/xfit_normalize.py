...
def parse_tmdwf_xfit_normalize_input(
    path: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> TMDWFXFitNormalizeInput:
    file_path = Path(path)
    ...
    return TMDWFXFitNormalizeInput(
        ...
        results_dir=(
            ...
            else Path(entries.get("results_dir", [file_path.parent / "results_tmdwf_xfit_normalized"])[0])
        ),
    )
...
def _load_xfit_samples(
    ...
    if not path.exists():
        raise FileNotFoundError(f"TMDWF x-fit sample table does not exist: {path}")
...
def _old_fourier_samples_from_bare_matrix_outputs(
    ...
    *,
    ...
    _, raw_fourier = compute_tmdwf_cosine_transform(
        bz_values,
    ...
    _, norm_fourier = compute_tmdwf_cosine_transform(
        bz_values,
...
def plot_tmdwf_xfit_normalized_x_dependence(
...
def run_tmdwf_xfit_normalization(
    input_file: str | Path,
    *,
    results_dir: str | Path | None = None,
) -> list[Path]:
    spec = parse_tmdwf_xfit_normalize_input(input_file, results_dir=results_dir)
    outputs: list[Path] = []
            ...
            for eta in spec.etalist:
                for bT in spec.bTlist:
                    ...
                    for x_value in x_values:
                        ...
                        q0_err.append(err)
                    outputs.append(
                        plot_tmdwf_xfit_normalized_x_dependence(
                            dataset_root
                            / "plots"
